...
def run(
    ...
    # Keep only cards where the grading spread covers the fee with room to profit:
    # PSA 9 price must exceed raw price by at least grading_fee + $5
    ...
    print(f"{len(opportunities)} total opportunities → top 50 go to image analysis")
    print(f"{'='*60}\n")

    # Step 6: eBay image analysis (final filter on top 50 by spread)
    final = opportunities
    ...
    if not skip_images and not opportunities.empty:
        ...
        _time.sleep(wait_s)
        n_opps = min(len(opportunities), 50)
