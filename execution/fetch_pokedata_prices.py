...
def _passes_price_filter(
    ...
    """
    Card must pass BOTH conditions:
      1. PSA 9 or PSA 10 price > min_graded_price
      2. (best_graded - raw - grading_fee) / (raw + grading_fee) >= min_roi
    """
...
def run(
    ...
    df = pd.DataFrame(results) if results else pd.DataFrame(
        columns=["card_name", "set_name", "card_number", "card_id",
                 "raw_price", "psa9_price", "psa10_price", "roi_at_best_grade", "source_url"])
